...
### extension/md/drjele_abandoned_ships_cleanup.xml
...
&lt;set_value name="global.$DrJeleAbandonedShipLifetime" exact="30min"/&gt; Those globals are also what the in-game options menu writes - see drjele_abandoned_ships_cleanup_options.xml.

Per-ship clocks live here rather than on the ships themselves: X4 refuses to attach script variables to a plain ship component ("Failed to set component.{...}.$var"), so the state is a table keyed by object, which is the idiom vanilla uses in md/lib_generic.xml.
$Clock.{$ship} == [first seen abandoned, last seen abandoned]

A ship dies on the first scan after its clock expires, so the scan interval is the error bar on the lifetime. Keep it small enough to stay a fifth of it, or a five minute lifetime would really mean five to ten.

A gap between two sightings larger than $StaleGap means the ship was claimed and abandoned again in between, so the clock restarts instead of expiring at once.

### extension/md/drjele_abandoned_ships_cleanup_options.xml

In-game options, through SirNukes Mod Support APIs. Everything that touches that API lives in this file, so if the API is not installed nothing here ever runs and the mod keeps working off the constants in drjele_abandoned_ships_cleanup.xml.

Both callbacks only ever write the global override variables the main script already reads, so the cleanup logic itself knows nothing about menus.

The API stores an option's value in the savegame under its $id, and hands it straight back to the widget as its start value. A stored value outside the slider's range fails widget validation and takes the whole Extension Options menu down with it, so the range and the unit of an option must never change under a $id that has already shipped - give it a new one instead. That is why this id carries its unit.
